...
ℹ️ The fastest random functions for real-time usage are in [vec.h](vec.h) with `srandf()` _(seed rand)_, `randf()` _(0-1)_, `randfc()` _(-1 to +1)_, `randfn()` _([normal distribution](https://en.wikipedia.org/wiki/Normal_distribution))_.
